...
module.exports={
    context:path.join(__dirname, 'resources/assets/build/js'),//根目录
    entry:{vendor:['jquery','bootstrap'],base:['jquery','bootstrap','./app'],shim:['./vendor/html5shiv/html5shiv.min','./vendor/response/respond.min','es5-shim','es6-shim']},
    output:{
        path:path.join(__dirname,'resources/assets/build/js/bundles'),
        filename:'[name].bundle.js',//同步加载的文件都被打包到bundel
        chunkFilename:'[id].chunk.js',//异步加载的文件都会被放到chunk,“chunks” which are loaded on demand.
        publicPath: 'http://asset.admin.mj.com/js/',//指定发布路径，也就是说异步加载的文件都会以这个路径为基础请求,可使用webpack-dev-server --content-base dist/js 指定到发布目录
