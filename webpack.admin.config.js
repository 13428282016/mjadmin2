...
module.exports={
    context:path.join(__dirname, 'resources/assets/build/js/admin'),//根目录
    entry:{vendor:['jquery','bootstrap','react'],base:['jquery','bootstrap','./app'],shim:['./vendor/html5shiv/html5shiv.min','./vendor/response/respond.min','es5-shim','es6-shim']},
    output:{
        path:path.join(__dirname,'resources/assets/build/js/admin/bundles'),
        filename:'[name].bundle.js',//同步加载的文件都被打包到bundel
        chunkFilename:'[id].chunk.js',//异步加载的文件都会被放到chunk,“chunks” which are loaded on demand.
        publicPath: 'http://assets.mj.kankan/com/js/admin',//指定发布路径，也就是说异步加载的文件都会以这个路径为基础请求,可使用webpack-dev-server --content-base dist/js 指定到发布目录
    ...
    plugins: [
      ...
      //  new webpack.optimize.CommonsChunkPlugin("common1.js",['demo3g','demo3a','./example.js']),//可以指定那个js需要合并
        new webpack.optimize.CommonsChunkPlugin("vendor", "vendor.js"),//所有引用属于vendor选项包含的js，这些js都会从他们的chunk或bundle移除，并合并到一个新的chunk，注意加载的时候要先加载这个chunk
